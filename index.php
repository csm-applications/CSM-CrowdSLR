
<!DOCTYPE html>


<html>

<?php include 'elements/head.php' ?>

<body class="container">

    <div >
        <div class="jumbotron" style="background:white	; text-align: center" >
            <h1 class="display-4">CrowdSLR</h1>
            <p class="lead">Support tool</p>
        </div>

        <div class="card" style="background-color: #fafafa; padding: 20px">

            <p>Welcome to the tool to support the selection of primary studies using crowdsource. This project was created in order to help researchers to select primary studies using crowdsource platforms.</p>

            <p>This software is free code and can be adapted to the researcher's needs. It is under license XXX and allows it to be used freely, however, when it is modified and made available, it is necessary to inform that its origin.</p>

        </div> 

        <div class="card" style="color: red; margin-top:20px; padding: 20px">
            <h3>To setup your application please use our <a href="config.php?redirect=wizard/wizard-step1">configuration wizard</a></h3>
        </div>


        <div class="card" style="color: red; margin-top:20px; padding: 20px">
            <h3><a href="config.php?redirect=results/overview">See the results</a></h3>
        </div>
    </div>
</body>
</html>

